...
function pickAccount(accountId) {
  ...
  if (!appid || !secret) {
    throw new Error(`公众号「${account.name || account.id}」缺少 APPID 或 AppSecret`);
  }
  return { account, appid, secret };
}

async function getAccessToken(accountId) {
  const { account, appid, secret } = pickAccount(accountId);
  const cache = store.getTokenFor(appid);
  if (cache && cache.token && cache.expire > Date.now() + 60_000) {
    return cache.token;
  }
  ...
  if (!data.access_token) {
    const label = account.name || appid;
    if (data.errcode === 40125) {
      throw new Error(`公众号「${label}」AppSecret 不正确（errcode 40125）。请到「微信公众平台 → 开发 → 基本配置」核对 AppSecret，或点「重置」生成新的，然后在配置页粘贴时注意别带空格/换行。`);
    }
    if (data.errcode === 40164 || data.errcode === 40137) {
      throw new Error(`公众号「${label}」服务器公网 IP 未加入白名单（errcode ${data.errcode}）。请到「微信公众平台 → 开发 → 基本配置 → IP 白名单」把状态页显示的公网 IP 加进去。`);
    }
    if (data.errcode === 40013) {
      throw new Error(`公众号「${label}」AppID 不正确（errcode 40013）。请到「微信公众平台 → 开发 → 基本配置」核对 AppID。`);
    }
    throw new Error(`获取 access_token 失败（${label}）: ${JSON.stringify(data)}`);
  }
  store.setTokenFor(appid, data.access_token, Date.now() + (data.expires_in - 300) * 1000);
...
async function uploadImageMaterial(buffer, filename = 'cover.jpg', accountId) {
...
async function uploadContentImage(buffer, filename = 'inline.jpg', accountId) {
  const token = await getAccessToken(accountId);
  const { mime, ext } = ensureSupportedForWechat(sniffImageType(buffer, filename));
  const safeName = filename.replace(/\.[^./\\]+$/, '') + '.' + ext;
  const form = buildForm(buffer, safeName, mime);
...
async function addDraft({ title, content, author, digest, thumbMediaId, contentSourceUrl, accountId }) {
  const token = await getAccessToken(accountId);
