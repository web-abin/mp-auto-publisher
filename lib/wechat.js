...
async function getAccessToken() {
  ...
  if (!cfg.appid || !cfg.secret) throw new Error('未配置 APPID/Secret');
  const cache = store.getTokenCache();
  if (cache.token && cache.expire > Date.now() + 60_000 && cache.appid === cfg.appid) {
    return cache.token;
  }
  const url = `${API}/token?grant_type=client_credential&appid=${cfg.appid}&secret=${cfg.secret}`;
