...
async function callAnthropic(cfg, userPrompt, { webSearch = false, newsContext = '', systemPrompt } = {}) {
...
async function callOpenAI(cfg, userPrompt, { webSearch = false, newsContext = '', systemPrompt } = {}) {
  if (webSearch) {
  ...
  const baseSystem = (typeof systemPrompt === 'string' && systemPrompt.trim()) ? systemPrompt : SYSTEM_PROMPT;
  const system = baseSystem + buildNewsDirective(newsContext);
  const body = {
...
async function generateArticle({ keyword, related = [], extra = '', webSearch = false, newsContext = '', systemPrompt = '' }) {
...
请按系统提示要求写一篇公众号文章，输出 JSON。`;
  const text = cfg.aiProvider === 'openai'
    ? await callOpenAI(cfg, userPrompt, { webSearch, newsContext, systemPrompt })
    : await callAnthropic(cfg, userPrompt, { webSearch, newsContext, systemPrompt });
