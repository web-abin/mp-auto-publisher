...
function buildReferenceDirective(referenceContext) {
  if (!referenceContext) return '';
  return `

【参考材料（用户提供的原文）】下面是用户指定的若干篇参考文章正文。你的任务是基于这些材料做"洗稿改写"——按系统提示里的风格写一篇全新的公众号文章：
- 这些材料是本次写作的核心信息来源，比泛泛而谈更重要；要尽可能消化里面的具体事实/数据/案例/人物/时间线/观点。
- 严禁照抄原文句式或段落，必须用自己的话重新组织、重新表达；句子结构、衔接方式、段落顺序都要不一样。
- 多篇材料要融会贯通，不是简单拼接；如果观点冲突，可以选边或并陈。
- 不要保留参考材料里的小标题/列表结构，按我们要求的结构重新写。
- 不要在文中出现"根据材料""参考文献""原文提到""综合多篇报道"之类的痕迹；也不要写出参考链接。
- 不要在输出里复述参考材料清单，只输出最终 JSON。
...
async function callAnthropic(cfg, userPrompt, { webSearch = false, newsContext = '', referenceContext = '', systemPrompt } = {}) {
  const baseUrl = cfg.aiBaseUrl || 'https://api.anthropic.com';
  const baseSystem = (typeof systemPrompt === 'string' && systemPrompt.trim()) ? systemPrompt : SYSTEM_PROMPT;
  const system = baseSystem
    + (webSearch ? WEB_SEARCH_DIRECTIVE : '')
    + buildNewsDirective(newsContext)
    + buildReferenceDirective(referenceContext);
  const body = {
...
async function callOpenAI(cfg, userPrompt, { webSearch = false, newsContext = '', referenceContext = '', systemPrompt } = {}) {
  if (webSearch) {
  ...
  const baseSystem = (typeof systemPrompt === 'string' && systemPrompt.trim()) ? systemPrompt : SYSTEM_PROMPT;
  const system = baseSystem + buildNewsDirective(newsContext) + buildReferenceDirective(referenceContext);
  const body = {
...
async function generateArticle({ keyword, related = [], extra = '', webSearch = false, newsContext = '', referenceContext = '', systemPrompt = '' }) {
  const cfg = store.getConfig();
  if (!cfg.aiKey) throw new Error('未配置 AI API Key');
  const hasRefs = !!referenceContext;
  const topicLine = keyword
    ? `核心关键词：${keyword}`
    : '核心关键词：（用户未提供，请直接从参考材料里提炼最有传播力的角度作为主线）';
  const userPrompt = `${topicLine}
相关/热搜词（自然融入即可，不必全用）：${related.join('、') || '无'}
${extra ? `补充要求：${extra}` : ''}
${hasRefs ? '\n请优先围绕"参考材料"洗稿改写，关键词只作为话题锚点；详见 system 中的「参考材料」段落。\n' : ''}
请按系统提示要求写一篇公众号文章，输出 JSON。`;
  const text = cfg.aiProvider === 'openai'
    ? await callOpenAI(cfg, userPrompt, { webSearch, newsContext, referenceContext, systemPrompt })
    : await callAnthropic(cfg, userPrompt, { webSearch, newsContext, referenceContext, systemPrompt });
...
// 为每个图位扩展 2 条不同视角的英文检索词。一次 AI 调用批量处理，失败则各自回退到原 query。
async function expandImageQueries(items) {
  if (!items || !items.length) return [];
  const cfg = store.getConfig();
  if (!cfg.aiKey) return items.map(it => [it.query]);

  const numbered = items.map((it, i) => {
    const ctx = (it.context || '').replace(/\s+/g, ' ').trim().slice(0, 280);
    return `${i + 1}. 原 query: "${it.query}"\n   段落: "${ctx}"`;
  }).join('\n');

  const userPrompt = `你是公众号视觉编辑，正在为一篇文章选配图准备检索词。下面是 ${items.length} 张图的"原始英文检索词 + 中文段落上下文"。请为每张图额外补充 2 条不同视角的英文检索短语，让候选图更丰富、更不平庸。

要求：
- 第二条：换主体/镜头（远景↔特写、室内↔街景、人物↔静物、整体↔细节 之类的视角切换）
- 第三条：换更抽象的表达（材质、纹理、光影、隐喻、氛围）
- 每条都遵守：主体（具体物件/场景） + 镜头/构图 + 光线/风格；不要 work / life / success / business 这种抽象大词
- 同一篇文章里不同图位的扩展词不要互相重复
- 必须英文

${numbered}

严格输出 JSON，不要 markdown，不要解释：
{ "queries": [ ["原 query", "alt1", "alt2"], ... ] }`;
  ...
  try {
    ...
    return items.map((it, i) => {
      const row = Array.isArray(arr[i]) ? arr[i].filter(s => typeof s === 'string' && s.trim()) : [];
      const all = [it.query, ...row].map(s => s.trim()).filter(Boolean);
      const seen = new Set();
      const dedup = [];
      for (const q of all) {
      ...
      return dedup.length ? dedup.slice(0, 3) : [it.query];
    });
  } catch (e) {
    console.error('expandImageQueries failed:', e.message);
    return items.map(it => [it.query]);
  }
}

module.exports = { generateArticle, expandImageQueries, DEFAULT_SYSTEM_PROMPT: SYSTEM_PROMPT };
