...
async function generateContent({
  ...
  log(`${writerLabel ? writerLabel + ' · ' : ''}${webSearch ? 'AI 生成文章中（已启用联网搜索）...' : 'AI 生成文章中...'}`);
  const article = await ai.generateArticle({ keyword, related, extra, webSearch, newsContext, systemPrompt: writerPrompt });
  ...
  log('搜索配图中（参考网页 + 多图源并行 + 多视角扩展 + 多模态挑图）...');
  ...
  // 微信公众号搜索基于 keyword 触发——即使没开 useNews 也提供参考图。
  const articleLinks = newsItems.map(n => n.link).filter(Boolean);
  ...
  const harvestP = wechatLinksP.then(wechatLinks => {
    ...
    log(`从 ${articleLinks.length} 条新闻 + ${wechatLinks.length} 条公众号文章抓取参考配图...`);
  ...
  async function pickFor(slot, queries) {
    ...
    const pool = [...availablePriority, ...merged];
    const pick = await images.pickBestImage(slot.query, pool, slot.context);
    if (pick && priorityPool.includes(pick)) usedFromPriority.add(pick);
    return pick;
  }

  const coverSlot = slots[0];
  const tagSlots = slots.slice(1);
  const coverPick = await pickFor(coverSlot, expanded[0] || [coverSlot.query]);
  const tagPicks = await Promise.all(
