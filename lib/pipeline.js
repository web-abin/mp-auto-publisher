...
async function generateContent({
  ...
  async function pickFor(slot, queries) {
    ...
    const merged = interleaveDedupe(Array.from(bySource.values()));
    ...
    // 候选列表：参考网页置顶 + 每个图源前几张；并把 AI 选中的图标记 selected。
    const candidates = [];
    const seen = new Set();
    ...
    // 若 AI 选中的不在候选里（比如来自 priorityPool 末尾），补到最前面。
    if (pick && !seen.has(pick)) candidates.unshift({ source: priorityPool.includes(pick) ? 'reference' : 'other', url: pick });

    return { selected: pick, candidates };
  }
